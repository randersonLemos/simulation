...
class Tables():
    ...
    def join(self, key, key1, key2, dell):
        ...
        df[Well_Keys.state()] = df[Well_Keys.state()].apply(lambda x: 1 if x else 0)
        df[Well_Keys.bhp()]   = df[Well_Keys.bhp()]  / 2.0
        df[Well_Keys.bhp_dot()]  = df[Well_Keys.bhp_dot()] / 2.0
    ...
    def to_csv(self, dir):
        ...
        for key in self._dic:
            ...
            if isinstance(self._dic[key], Well_Table):
                ...
                df.fillna(0).to_csv(dir / '{}.csv'.format(key.upper()))
