...
class TrainData(Aux):
    def __init__(self, X, y, iteration, mask, nlargest):
    ...
    def _processing(self):
        ...
        index = self.y.sort_values(by='NPV', ascending=False).index
        self.X = self.X.loc[index, :]
        self.y = self.y.loc[index, :]

        nlargest = (self.y['NPV'] / 1000).astype('int').nlargest(self.nlargest, keep='all')
