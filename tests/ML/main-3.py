...
if __name__ == "__main__":
    ...
    for i in range(1, 20):
        ...
        _probabilities = np.zeros((100, 1), dtype='float32')
        n_models = 0
        _n_models = 0
        for j in range(20):
            stg  = '\n\n\n'
            ...
            _cl = ClassifiedData(ted, probs, 0.40)
            if (_cl.y['CLASS'] == 1).sum() > 65 and (_cl.y['CLASS'] == 1).sum() < 80:
                stg  = '\n\n\n'
                stg += 'Model {} satisfy condition'.format(j)
                stg += '\n\n\n'
                print(stg)
